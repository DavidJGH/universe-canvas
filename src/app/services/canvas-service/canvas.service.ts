...
export class CanvasService {
  ...
  constructor(private readonly sanitizer: DomSanitizer, private readonly firestore: AngularFirestore){
  }

  public updateCanvasContinuously(canvas: HTMLCanvasElement): void {
    const context = canvas.getContext('2d');

    this.firestore.doc<{width: number, height: number, content: string}>('/canvas/uvXsZg1YPNaDhAtpxTcK').valueChanges().subscribe((document) => {
      if (document) {
